...
class UserRegTest extends DuskTestCase
{
    ...
    public function testUserRegEmptyInputTextbox()
    {
        $this->browse(function (Browser $browser)
        {
            $browser->visit($this->user_reg_url)
                    ...
                    ->click('button[type="submit"]')
                    ->pause(1000)
                    ->assertSee(Message::getMessage(Message::ERROR_001, [Util::langtext('USER_L_002')]))
                    ->assertSee(Message::getMessage(Message::ERROR_001, [Util::langtext('USER_L_021')]))
                    ->assertSee(Message::getMessage(Message::ERROR_001, [Util::langtext('USER_L_021')]))
                    ->assertSee(Message::getMessage(Message::ERROR_001, [Util::langtext('USER_L_004')]))
                    ->assertSee(Message::getMessage(Message::ERROR_001, [Util::langtext('USER_L_005')]));
        });
    }
    ...
    */
     ...
     */
      ...
      public function testCreatedAdminUser(){
        $this->browse(function (Browser $browser)
        {
           ...
           $browser->visit($this->user_reg_url)
                   ...
                   ->type('password_confirmation', $password)
                   ->pause(1000)
                   ->click('label[for="system-admin-flag"]')
                   ->select('status', '1')
     ...
     /**
      * Test case: Check redirection URL when キャンセル button is pressed and confirmed cancellation
      */
     ...
     /**
      * Test case: Check response when キャンセル button is pressed and denied cancellation
      */
      public function testIgnoreCancelRegistration(){
        $this->browse(function (Browser $browser)
        {
           $browser->visit($this->user_reg_url)
                   ->click('#user-form-cancel')
                   ...
                   ->assertSee(Message::getMessage(Message::INFO_005))
                   ->click('.btn-secondary');
            $this->assertStringContainsString($this->user_reg_url, $browser->driver->getCurrentURL());
        });
     }
