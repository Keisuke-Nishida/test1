        ...
        "columns": [
            ...
            { "data": "name" },
            {
                ...
                "render": function(data, type, row) {
                    return get_list_link("edit", 0, "/admin/schedule/edit/" + row["id"], "list-button", row["id"]);
                }
            },
            { "data": "type" },
            { "data": "start_time" },
            ...
            { "data": "last_run_time" },
            {
                ...
                "orderable": false,
                "render": function(data, type, row) {
                    return get_list_link("remove", row["id"], "/admin/schedule/delete_single", "list-button");
                }
            }
        ],
    });
...
$(document).ready(function() {
    ...
    $("#schedule-detailed-search-submit").click(function() {
        $("#schedule-table").DataTable().destroy();
        search_main_list();
    });

    $("#schedule-detailed-search-reset").click(function() {
        clear_search_fields();
        $("#schedule-table").DataTable().destroy();
        search_main_list();
    });

    $("#schedule-select-all").click(function() {
        if ($("#schedule-select-all").is(":checked")) {
    ...
    $("#schedule-multiple-delete-button").click(function(e) {
        $("#confirm_url").val($(this).data("url"));
    ...
    $("#schedule-form-cancel").click(function(e) {
        $("#confirm_url").val($(this).data("url"));
        $("#confirm_modal").modal("show");
    });
});
